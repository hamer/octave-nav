function test_fuse()
    ...
    dcm = rpy2dcm([ 0; 0; 90 ] * deg2rad); % initial values
    shift = [ 0; 0; 1 ];

    eps = 1e-1;
    for i = 1:5
        disp(''); disp([ '=========== Run C ===========' ]);
        [ cdcm, cshift ] = usbl_calib(shift_src(src, sdcm, shift), sdcm, dcm * xyz, 29);
        ...
        if norm(cshift) < eps && norm(cdcm - eye(3)) < eps
            break;
        end
