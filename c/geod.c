...
void ecef2geod(const double *ecef, double *geod) {
    ...
    const double phi = atan2(ecef[2] + Se2 * Sb * pow(sin(q), 3.0),
                             p - Sa * Se * Se * pow(cos(q), 3.0));
    const double h = p / cos(phi) - get_n(phi); // FIXME: handle division by zero
...
double geod2scale(const double *geod) {
    return 1.0 / cos(geod[0] * deg2rad); // FIXME: handle division by zero
}
